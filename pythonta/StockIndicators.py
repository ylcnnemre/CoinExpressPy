...
def TVGet(name, exchange, interval, nbars=100):
    ...
    if interval in interval_mapping:
        ...
        while retries > 0:
            try:
                data = tv.get_hist(symbol=name, exchange=exchange,
                                   interval=mapped_interval, n_bars=nbars)
                data = data.reset_index()
                return data
            except Exception as e:
                retries -= 1
                print(
                    f"An error occurred: {e}. Retrying {retries} more times.")
...
def smma(values, period):
    ...
    smma = pd.Series(smma, index=values.index if isinstance(
        values, pd.Series) else None)
    return smma
...
def salma(series, length=10, smooth=3, mult=0.3, sd_len=5):
    ...
    upper = baseline + dev
    lower = baseline - dev
    cprice = np.where(series > upper, upper, np.where(
        series < lower, lower, series))
...
def xsa(src, length, weight):
    ...
    for i in range(length, len(src)):
        sumf[i] = np.nan_to_num(sumf[i - 1]) - \
            np.nan_to_num(src[i - length]) + src[i]
        ma[i] = np.nan if np.isnan(src[i - length]) else sumf[i] / length
        xsa[i] = ma[i] if np.isnan(
            xsa[i - 1]) else (src[i] * weight + xsa[i - 1] * (length - weight)) / length
...
def vwap(high, low, close, volume):
    ...
    length = 1
    tp = (high + low + close) / 3
    vwap = (tp * volume).rolling(window=length).sum() / \
...
def mfi(high, low, close, volume, window=14):
    ...
    positive_money_flow = (
        money_flow * (close > close.shift(1))).rolling(window=window).sum()
    negative_money_flow = (
        money_flow * (close < close.shift(1))).rolling(window=window).sum()
...
def cmf(high, low, close, volume, length=20):
    ...
    cmf = money_flow_volume.rolling(
        window=length).sum() / volume.rolling(window=length).sum()
    return cmf
...
def mfi(high, low, close, volume, period=14):
    ...
    positive_flow = (raw_money_flow * (typical_price >
                     typical_price.shift(1))).rolling(window=period).sum()
    negative_flow = (raw_money_flow * (typical_price <
                     typical_price.shift(1))).rolling(window=period).sum()
...
def dss_bresser_scalper(high, low, close, ema_period=8, stoc_period=13):
    ...
    lowrange_ = mitbuffer.rolling(
        window=stoc_period).min().fillna(stoc_period * 1000)
...
def nadaraya_watson_envelope(data, bandwidth, mult=3.0):
    ...
    sae = (df['close'] - nw_estimator_series).abs().rolling(window=n,
                                                            min_periods=1).mean() * mult
...
def Keltner_Signal(data, window=20, mult=2):
    ...
    df['Entry'] = (df['close'] > df['Lower Channel']) & (
        df['close'].shift(1) < df['Lower Channel'])
    df['Exit'] = (df['close'] > df['Upper Channel']) & (
        df['close'].shift(1) < df['Upper Channel'])
    return df
...
def SqueezeMomentum(data, mult=2, length=20, multKC=1.5, lengthKC=20):
    ...
    df['Squeeze'] = (df['lowerBB'] < df['lowerKC']) & (
        df['upperBB'] > df['upperKC'])
    return df
...
def Psar_Signal(data, initial_af=0.02, step_af=0.02, max_af=0.2):
    ...
    parabolic = psar(df['high'], df['low'], df['close'],
                     initial_af, step_af, max_af)
...
def Supertrend_Signal(data, SENSITIVITY=3, ATR_PERIOD=14):
    ...
    for i in range(1, len(df)):
        if (df.loc[i, 'close'] > df.loc[i - 1, 'ATRTrailing']) and (df.loc[i - 1, 'close'] > df.loc[i - 1, 'ATRTrailing']):
            df.loc[i, 'ATRTrailing'] = max(
                df.loc[i - 1, 'ATRTrailing'], df.loc[i, 'close']-df.loc[i, 'nLoss'])

        elif (df.loc[i, 'close'] < df.loc[i - 1, 'ATRTrailing']) and (df.loc[i - 1, 'close'] < df.loc[i - 1, 'ATRTrailing']):
            df.loc[i, 'ATRTrailing'] = min(
                df.loc[i - 1, 'ATRTrailing'], df.loc[i, 'close']+df.loc[i, 'nLoss'])
...
def OTT_Signal(data, prt, prc):
    ...
    df['ud1'] = np.where(df['close'] > df['close'].shift(
        1), (df['close'] - df['close'].shift()), 0)
    df['dd1'] = np.where(df['close'] < df['close'].shift(
        1), (df['close'].shift() - df['close']), 0)
    ...
    for i in df['UD']:
        def maxlongstop():
            df.loc[(df['newlongstop'] > df['longstop'].shift(1)),
                   'longstop'] = df['newlongstop']
            df.loc[(df['longstop'].shift(1) > df['newlongstop']),
                   'longstop'] = df['longstop'].shift(1)
            return df['longstop']

        def minshortstop():
            df.loc[(df['newshortstop'] < df['shortstop'].shift(1)),
                   'shortstop'] = df['newshortstop']
            df.loc[(df['shortstop'].shift(1) < df['newshortstop']),
                   'shortstop'] = df['shortstop'].shift(1)
            return df['shortstop']

        df['longstop'] = np.where(
            ((df['Var'] > df['longstop'].shift(1))), maxlongstop(), df['newlongstop'])
        df['shortstop'] = np.where(
            ((df['Var'] < df['shortstop'].shift(1))), minshortstop(), df['newshortstop'])
    ...
    df['OTT'] = np.where(df['Var'] > df['MT'], (df['MT'] *
                         (200 + percent) / 200), (df['MT'] * (200 - percent) / 200))
...
def Banker_Fund_Trend(data):
    ...
    part_1 = (close - low.rolling(window=27).min()) / \
        (high.rolling(window=27).max() - low.rolling(window=27).min()) * 100
    fundtrend = ((3 * xsa(part_1, 5, 1) - 2 *
                 xsa(xsa(part_1, 5, 1), 3, 1) - 50) * 1.032 + 50)
...
def Alpha_Trend_Signal(data, mult=1, n1=14):
    ...
    for i in range(1, len(df)):
        if df['mfi'].iloc[i] >= 50:
            alpha_trend_values.append(
                max(df['upT'].iloc[i], alpha_trend_values[i-1]))
        else:
            alpha_trend_values.append(
                min(df['downT'].iloc[i], alpha_trend_values[i-1]))
...
def Ichimoku_Signal(data, n1=9, n2=26, n3=52, n4=26, n5=26):
    ...
    df['Entry'] = (df['close'] > df['senkou_A']) & (df['close'] > df['senkou_B']) & (
        df['close'] > df['kijunsen']) & df['chikou'] > df['close']
    df['Exit'] = (df['close'] < df['senkou_A']) | (df['close'] < df['senkou_B']) | (
        df['close'] < df['kijunsen']) | (df['chikou'] < df['close'])
...
def Divergence(data, DivCheck, order=3):
    ...
    data = data.reset_index()
    hh_pairs = argrelextrema(
        df['close'].values, comparator=np.greater, order=order)[0]
    hh_pairs = [hh_pairs[i:i+2] for i in range(len(hh_pairs)-1)]

    ll_pairs = argrelextrema(
        df['close'].values, comparator=np.less, order=order)[0]
